...
class SettingsViewController: UIViewController {
    ...
    func setFooterData() {
        ...
        if let thisVersion = Bundle.main.infoDictionary?["CFBundleShortVersionString"] as? String {
            self.versionLabel.text = "Storage v" + thisVersion
        } else {
            self.versionLabel.text = "Storage v?.?.?"
        }
        let date = Date()
        let calendar = Calendar.current
        let year = calendar.component(.year, from: date)
        if year > 2018 {
            self.copyrightLabel.text = "© 2018-" + String(year) + " ka yuan"
        } else {
            self.copyrightLabel.text = "© 2018 ka yuan"
        }
    }
    func rateApp() {
